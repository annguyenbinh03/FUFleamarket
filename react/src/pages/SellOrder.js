...
import { acceptBuyRequestOrdersAPI, denyBuyRequestOrdersAPI, getSellOrdersAPI } from "../api/order";
            ...
            {orders?.map((order) => (
              <div key={order.order?.orderId} className="row mb-3 p-2 order">
                <div className="col-12 border-bottom ps-2 py-2">
                  <span className="fs-5">
                    <i className="fa fa-home" aria-hidden="true"></i> Nơi bán
                  </span>
                </div>
                      ...
                      <div className="">
                        {order.order?.status === 0 ? (
                          <div className="d-flex justify-content-center mt-2">
                            <button className="btn btn-info mx-3" onClick={()=> handleAcceptOrder(order.order?.orderId)}>
                              Xác nhận đơn hàng
                            </button>
                            <button className="btn btn-danger mx-3 " onClick={()=> handleDenyOrder(order.order?.orderId)}>
                              Hủy đơn hàng
                            </button>
                          </div>
                ...
                <div className="col-4 col-md-4 pt-3 order_info">
                  ...
                  <div className="d-flex justify-content-center mt-2">
                    <button className="btn btn-warning">Xem Feedback</button>
                  </div>
                </div>
              </div>
            ))}
