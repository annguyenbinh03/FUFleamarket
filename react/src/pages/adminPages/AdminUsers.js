...
import { adminBanAccount, adminUnBanAccount, getAllUserAPI } from "../../api/user";
...
function AdminUsers() {
  ...
  const banAccount = async (userid) =>{
    await adminBanAccount(auth.accessToken, userid);
    fetchData();
  }

  
  const unbanAccount = async (userid) =>{
    await adminUnBanAccount(auth.accessToken, userid);
    fetchData();
  }
    ...
    <div className="admin_page">
      ...
      <div className="admin_main container-fluid d-flex justify-content-center p-0 pt-3 mt-5">
        ...
        <div className="main-content w-87 pt-3 px-4">
          ...
          <p className="">Danh mục tất cả người dùng </p>
                        ...
                        {user.roleId === 1 && (
                          <button
                            className="btn btn-danger mx-2"
                              onClick={() =>
                                banAccount(user.userId)
                              }
                          >
                            Ban
                          </button>
                        )}
                        {user.roleId === 2 && auth.roles?.[0] === 3 && (
                          <button
                            className="btn btn-danger mx-2"
                            onClick={() =>
                              banAccount(user.userId)
