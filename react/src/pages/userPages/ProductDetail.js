  ...
  return (
    <div>
      ...
      <section className="product-details spad">
             { (data?.product?.status === 0 || data?.product?.status === 2 ) && (auth?.roles?.[0] !== 2 && auth?.roles?.[0] !== 3 ) ? (
               ...
               Sản phẩm đang chờ admin duyệt
                ...
                <div className="col-lg-4 col-md-4 px-4">
                  ...
                  {auth?.id === data?.sellerId || auth.roles?.[0] !== 1 ? (
                    <></>
                  ) : (
                    <>    
                      {data?.product?.dealType ? (
                        <button className="chat_button  w-100  px-3 mt-5">
                          <Link
