...
const TradingOrderRequest = () => {
  ...
  const fetchOrder = async () => {
    try {
      var response = await getMyTradingOrderRequestAPI(
        auth.accessToken,
        tab,
        sortBy
      );
  ...
  const handleRejectOrder = async (orderId) => {
    try {
      await user2RejectRequest(auth.accessToken, orderId);
      fetchOrder();
      ...
      <section className="trading-order spad">
        <div className="container bg-white px-1 py-3">
          <div className="product_container px-4 py-2">
            <h3 className="mb-4 pb-2 fw-bold">Yêu cầu trao đổi</h3>
                          ...
                          {order?.status === 0 ? (
                            <div className="d-flex flex-column align-items-center mt-4">
                              <button
                                ...
                                onClick={() =>
                                  handleAcceptOrder(order?.tradingOrderId)
                                }
                              >
                                Bắt đầu trao đổi
                              </button>
